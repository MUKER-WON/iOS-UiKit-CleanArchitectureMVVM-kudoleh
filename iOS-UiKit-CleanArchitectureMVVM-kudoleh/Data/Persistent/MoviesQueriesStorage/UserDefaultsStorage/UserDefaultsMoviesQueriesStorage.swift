...
final class UserDefaultsMoviesQueriesStorage {
    ...
    init(
        ...
        backgroundQueue: DispatchQueueType
    ...
    private func fetchMoviesQueries() -> [MovieQuery] {
        if let queriesData = userDefaults.object(
            forKey: recentsMoviesQueriesKey
        ) as? Data {
            if let movieQueryList =
                try? JSONDecoder().decode(
                    MovieQueriesListUserDefaults.self,
                    from: queriesData
                ) {
    ...
    private func persist(moviesQueries: [MovieQuery]) {
        ...
        let movieQueryUserDefaults = 
        moviesQueries.map(MovieQueryUserDefaults.init)
        if let encoded = 
            try? encoder.encode(MovieQueriesListUserDefaults(
                list: movieQueryUserDefaults
            )) {
    ...
    private func cleanUpQueries(
        ...
        removeQueries(limit: maxStorageLimit, in: &queries)
    }
    ...
    private func removeQueries(
        ...
        queries = queries.count <= limit
        ? queries
        : Array(queries[0..<limit])
    }
}
...
extension UserDefaultsMoviesQueriesStorage: MoviesQueriesStorage {
    func fetchRecentsQueries(
        ...
        backgroundQueue.async { [weak self] in
            ...
            queries = queries.count < self.maxStorageLimit
            ? queries
            : Array(queries[0..<maxCount])
